...
//receive剩余的未receive完的HTTP响应data
...
{
	//	ASSERT(psock);
	if(m_respcode<=0) return false; //必须已经获取和HTTP response header
...
{
	//	ASSERT(psock);
	if(m_respcode<=0) return 0; //必须已经获取和HTTP response header
	...
	if(m_respcode==206) //response content为部分内容
	{//获取起始位置 //format Content-Range: bytes %d-%d/%d
		it=m_httprsp_HEADER.find("Content-Range");
		if(it!=m_httprsp_HEADER.end()) startPos=atol((*it).second.c_str()+6);
	}
	fp=(startPos>0)?fopen(filename,"ab"):fopen(filename,"wb");
	if(fp==NULL) return 0; else if(startPos>0) fseek(fp,startPos,SEEK_SET); //指针移到specified的位置
/*  //yyc remove 2007-12-20 begin
	...
	{//获取起始位置 //format Content-Range: bytes %d-%d/%d
		...
		if(it!=m_httprsp_HEADER.end()) startPos=atol((*it).second.c_str()+6);
		::fseek(fp,startPos,SEEK_SET); //指针移到specified的位置
	}
...
//success返回响应代码<0发生error 0:unknown的响应码 
SOCKSRESULT httpResponse::recv_rspH(socketTCP *psock,time_t timeout)
{
//	ASSERT(psock);
	const char * pFoundTerminator=NULL; //HTTP response header是否receive完毕
	...
	while(	psock->status()==SOCKS_CONNECTED ) 
	{
		...
		if(iret<0) break;
		if(iret==0){ cUtils::usleep(MAXRATIOTIMEOUT); continue; }//==0表明receivedata流量超过限制
		buffer.len()+=iret; buffer[buffer.len()]=0;
		if( (pFoundTerminator=strstr(buffer.str(),"\r\n\r\n")) )
		{	
			//如果web service返回响应码100 ，则说明其收到一部分data，但未receive完毕，正等待receive后续的data
	...
	*(char *)pFoundTerminator='\r';
	...
	init_httprsp(); //initializationhttp 请求对象的变量
	...
	{//判断响应data是否receive完毕
		...
		if(m_httprsp_lContentlen>0 && receivedBytes>=m_httprsp_lContentlen)
			m_httprsp_bReceiveALL=true; //响应data已经receive完毕
...
SOCKSRESULT httpResponse::send_rspH(socketTCP *psock,int respcode,const char *respDesc)
{
	...
	{
		//206指明传输的是部分data，在响应头里必须包含 Content-Range: bytes %d-%d/%d,但不必包含Accept-Ranges
...
//send文件多个区间断的内容，success返回SOCKSERR_OK
...
SOCKSRESULT httpResponse::sendfile(socketTCP *psock,const char *filename,MIMETYPE_ENUM mt,long* lpstartPos,long* lpendPos,int iRangeNums)
{
	...
	m_httprsp_lContentlen=lContentlen + 2 * iRangeNums; //还要包含每个data块最后的\r\n
	sr=send_rspH(psock,206,"Partial content");
	if(sr<0) return sr;
...
//				}
				//新设置的cookie，formatSet-Cookie: ssic=aaaa; expires=Thu, 13-Dec-2007 04:02:04 GMT; path=/
	...
	while(true)
	{
		...
		if( (ptr=strchr(ptrStart,'=')) )
		{
			*ptr++=0;
			...
			sName.assign(::_strlwr(ptrStart)); //name应不区分size写，因此全转换为小写
			sVal.assign(ptr);
