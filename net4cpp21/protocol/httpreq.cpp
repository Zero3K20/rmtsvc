...
//Content-Type: application/x-www-form-urlencoded   //form 提交data
...
//Content-Type: text/xml; charset=utf-8
//Content-Type: application/x-www-form-urlencoded; charset=utf-8   //form 提交data
//返回提交content的编码方式
...
{//如果此HTTP request不以匿名方式访问web service，则在此设置访问的account额password
...
//send HTTP request,success返回SOCKSERR_OK
SOCKSRESULT httpRequest::send_req(socketTCP *psock,const char *lpszurl)
{
//	ASSERT(psock);
	if(lpszurl) parseURL(lpszurl); //先将request parameters和url分离，分别保存再param_GET和url中
	//假如有POST data应先编码POST data
...
//		m_httpreq_iType=(m_httpreq_postdata.len()>0)?HTTP_REQ_POST:HTTP_REQ_GET;
	...
	int l; cBuffer contentBuffer(1024); //HTTP request缓冲
	EncodeFirstRequestLine(contentBuffer);
	
	if(m_bParseParams){//yyc add 2007-12-10 对代理转发不做判断
		std::map<std::string,std::string>::iterator it=m_httpreq_HEADER.find("Content-Length");
		if( it==m_httpreq_HEADER.end() && //是否有Contentdata
...
//		if( (it=m_httpreq_HEADER.find("Accept"))==m_httpreq_HEADER.end()) //yyc modify 2007-12-10
	...
	if(psock==NULL) //仅仅获取要send的data，并步send
	{
...
//	ASSERT(psock);
	...
	while(	psock->status()==SOCKS_CONNECTED ) 
	{
		...
		*(pbuf+iret)=0; buffer.len()+=iret;
		...
		if(buffer.Space()<256) //预分配一定的空间，以便完整receive请求头
	...
	*(char *)pFoundTerminator='\r';
	...
	init_httpreq(); //initializationhttp 请求对象的变量
	if(ParseRequest(buffer.str())==HTTP_REQ_UNKNOWN) return HTTP_REQ_UNKNOWN;
	if(m_httpreq_lContentlen>0)
	{//判断postdata是否receive完毕
		pbuf=(char *)pFoundTerminator+4; //post参数起始address
		...
		{//postdata已经receive完毕
			...
			else //如果不解码或不是form表单提交的编码data则保存到m_httpreq_postdata
			{
...
//receiveBytes - receivespecified的字节length  <=0:receive所有未完data
//success返回真,否则返回假
...
//			RW_LOG_DEBUG("[httpRequest] - Receive Data return %d\r\n",l);
			if(l<0) break;
			if(l==0){ cUtils::usleep(SCHECKTIMEOUT); continue; }//==0表明receivedata流量超过限制
...
//parse HTTP request头,返回HTTP requesttype
...
//parse HTTP request第一行的data，返回HTTP requesttype
...
//	ASSERT(lpszLine);
	...
	while(*ptrStart==' ') ptrStart++;//delete前导空格
	//获取request URL
	...
	if(m_httpreq_iType==HTTP_REQ_CONNECT)
	{
		...
		m_httpreq_strUrl="";
	}else{ //如果是HTTP代理，则send过来的URL中带有主机info，即完整的URLaddresshttp://xxxx
		char c=0;
		...
		{//"PHost"头是自define的，主要是为了给proxy service获取connect主机的info用
	...
	m_httpreq_dwVer=0; //否则必定包含协议version
	while(*ptrStart==' ') ptrStart++;//delete前导空格
...
//		while(len>0 && (*(ptr+len-1)=='\r' || *(ptr+len-1)=='\n') ) len--; //暂时先不handle，因为这样会丢掉IEsend的data\r\n
		iret=cCoder::mime_decode(ptr,len,ptr);
		if(ifutf_8)
			iret=cCoder::utf8_decode(ptr,iret,ptr);
		//name应不区分size写，因此全转换为小写
										  ...
										  std::string &username,std::string &password)
{
	...
	{ //当作没有编码的passwordhandle,format user:pswd
