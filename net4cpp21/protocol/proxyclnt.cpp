...
//connectspecified的TCP服务。success返回localsocketport>0，否则返回error
SOCKSRESULT socketProxy :: Connect(const char *host,int port,time_t lWaitout)
{
	...
	if(m_proxytype==PROXY_NONE) //直接connectspecified主机
		return socketTCP::Connect(host,port,lWaitout);
	//否则先connectproxy service器
...
//[in] svrIP/svrPort localUDPport和IP
//[out] svrIP/svrPort返回socks服务开的port和IPaddress
...
//在SOCKS Server上开一个侦听服务,svrIP/svrPort返回socks服务开的port和IPaddress
//[in] svrIP,告诉socks服务开的port等待并允许那个源IPconnect，svrPort无意义
SOCKSRESULT socketProxy :: sendReq_Bind(std::string &svrIP,int &svrPort,time_t lWaitout)
{
	...
	if(ipAddr==INADDR_NONE) return SOCKSERR_HOST;//必须是有效的IPv4address
	...
	else if(m_proxytype==PROXY_SOCKS5)
	{
		...
		buf[2]=0x0; buf[3]=0x1; //指明是ipv4的address
...
//sendsocks Bind请求后启动的等待connect到达的线程(即第二个Bind响应)
//sendBind请求后如果有一个合法用户connect到socks服务上则产生onBinded事件
//此时可进行data交互，就好像用户直接连到local服务的port上而不是socks服务开的port上一样
...
SOCKSRESULT socketProxy :: sendReq_Connect(const char *host,int port,time_t lWaitout)
{
	...
	if(m_proxytype==PROXY_SOCKS5)
	{
		...
		if(ipAddr==INADDR_NONE){
			buf[3]=0x03;
			buf[4]=sprintf(buf+5,"%s",host)+1; //包含NULLend符号
	...
	{
		//sendsocks4 CONNECT请求，请求connectspecified的主机
		//socks4A协议可以specifiedremoteconnect主机的域名而不必是ip
		//format如下：(原sock4协议IPaddress部分填入0.0.0.x),即buf[4]=buf[5]=buf[6]=0,buf[7]=x
		//[version] [命令] [port] [0.0.0.1] [0] [hostdomain] [0]
		...
		unsigned long ipAddr=INADDR_NONE;
		if(!m_ipv6) ipAddr=socketBase::Host2IP(host);
		if(ipAddr==INADDR_NONE) //也许resolve hostname无效，交给server端parse
		{
			ipAddr=0x01000000;
			::memcpy((void *)(buf+buflen),(const void *)&ipAddr,sizeof(ipAddr));
	...
	if(m_proxytype==PROXY_SOCKS5)
		sr=buf[1]; //如果返回success则buf[1]应为0
	else if(m_proxytype==PROXY_HTTPS)
	{
...
bool socketProxy :: socks5_Negotiation(time_t lWaitout)
{
	...
	{//服务方要求authentication
		...
		sr=this->Receive(buf,2,lWaitout);
		if(sr!=2) return false;
		if(buf[1]!=0) return false; //认证failure
	}//?if(buf[1]==0x02)
	else if(buf[1]!=0) //not supported要求的认证方式
	{
...
//sendUDP代理请求，success返回SOCKSERR_OK(0) (仅仅socks5代理协议支持UDP代理)
//在SOCKS Server上开一个侦听服务,svrIP/svrPort返回socks服务开的port和IPaddress
//[in] svrIP/svrPort localUDPport和IP
