...
namespace net4cpp21
{
	typedef enum //debug output level
	{
		...
		LOGLEVEL_WARN,//输出warninginfo
		LOGLEVEL_INFO,//输出用户记录info
		LOGLEVEL_ERROR,//输出errorinfo
		LOGLEVEL_FATAL //输出致命errorinfo
	}LOGLEVEL;
	...
	class cLogger
	{
		...
		TCHAR m_fileopenType[4]; //default未覆盖打开方式"w"
		bool m_bOutStdout;//是否同步output to console
		bool m_bOutputTime;//是否打印输出time
		CB_LOG_OUTPUT *m_pcallback;
		long	m_lcbparam; //callback函数用户参数
		static cLogger *plogger;//静态日志类实例指针
		cLogger(); //不允许create此类实例
