...
MIMETYPE_MHT,
MIMETYPE_UNKNOWED,
MIMETYPE_NONE		//sendfile时not再设定Content-Type,由user自己设定
}MIMETYPE_ENUM;
...
/*
100 Continue：初始的request已经接受，客户应whencontinuesendrequest的其余partial。 
...
　　202 Accepted：已经接受request，但handle尚未complete。 
...
　　205 Reset Content：没有新的内容，但浏览器shouldreset它所显示的内容。用来强制浏览器清除表单输入内容。 
　　206 Partial Content：客户send了一个带有Range头的GETrequest，servercomplete了它。 
　　300 Multiple Choices：客户request的文档可以at多个bit置found，这些bit置已经atreturn的文档内列出。ifserver要提出优先选择，则shouldatLocation应答头specifies。 
　　301 Moved Permanently：客户request的文档at其他地方，新的URLatLocation头中给出，浏览器should自动地访问新的URL。 
　　302 Found：class似于301，但新的URLshould被视为临时性的替代，而notyes永久性的。注意，atHTTP1.0中对应的statusinfoyes“Moved Temporatily”，而HttpServletResponse中相应的constantyesSC_MOVED_TEMPORARILY，而notyesSC_FOUND。出现该status代码时，浏览器能够自动访问新的URL，therefore它yes一个很有用的status代码。为此，Servlet提供了一个专用的method，即sendRedirect。使用response.sendRedirect(url)比使用response.setStatus(response.SC_MOVED_TEMPORARILY)andresponse.setHeader("Location",url)更好。注意这个status代码有时候可以and301替换使用。　　 
...
　　307 Temporary Redirect：and302（Found）相同。许多浏览器会error地response302应答进行重定向，即使原来的requestyesPOST，即使它实际上只能atPOSTrequest的应答yes303时才能重定向。由于这个原因，HTTP 1.1新增了307，以便更加清除地区分几个status代码：when出现303应答时，浏览器可以跟随重定向的GETandPOSTrequest；ifyes307应答，则浏览器只能跟随对GETrequest的重定向。注意：HttpServletResponse中没有为该status代码提供相应的constant。 
　　400 Bad Request：request出现语法error。 
　　401 Unauthorized：客户试图未经authorization访问受password保护的页面。应答中会contains一个WWW-Authenticate头，浏览器据此显示username字/password对话框，然后at填写合适的Authorization头后再次发出request。 
　　403 Forbidden：资源not可用。server理解客户的request，但拒绝handle它。通常由于server上fileordirectory的permissionsset导致。 
　　404 Not Found：无法foundspecifiedbit置的资源。这也yes一个常用的应答，HttpServletResponse专门提供了相应的method：sendError(message)。 
　　405 Method Not Allowed：request method（GET、POST、HEAD、DELETE、PUT、TRACE等）对specified的资源not适用。 
　　406 Not Acceptable：specified的资源已经found，但它的MIMEtypeand客户atAccpet头中所specified的not兼容。 
...
　　409 Conflict：通常andPUTrequest有关。由于requestand资源的currentstatus相冲突，thereforerequestnot能success。 
　　410 Gone：所request的文档已经not再可用，而且servernot知道should重定向到哪一个address。它and404的not同at于，return407表示文档永久地离开了specified的bit置，而404表示由于unknown的原因文档not可用。 
　　411 Length Required：servernot能handlerequest，除非客户send一个Content-Length头。 
...
　　416 Requested Range Not Satisfiable：servernot能满足客户atrequest中specified的Range头。 
　　500 Internal Server Error：server遇到了意料not到的情况，not能complete客户的request。 
...
　　504 Gateway Timeout：由作为代理or网关的server使用，表示not能及时地从remoteserver获得应答。 
...
getmethod用于getURI资源，yes最为常用的一种method。

postmethod用于向specifiedURI提交内容，server端response其行为，该method也极为常用。

headmethod向URIsendrequest，仅仅只需要获得response的protocol头。

putmethod用于向URIsendrequest，若URInot存at，则要求server端根据requestcreate资源。whenURI存at时，server端必须接受request内容，将其作为URI资源的modify后version。

deletemethod用于deleteURI标识的specified资源。

tracemethod用于激活server端对request的循环反馈，反馈作为HTTP response的正文内容被传输回client。
