...
typedef long HCHATSESSION; //define聊天会话句柄，实际此句柄pointer tocContactorobject
...
{ 
	...
	MSNINVITE_TYPE_PICTURE, //头像request only for msnc1
	MSNINVITE_TYPE_FILE,//file transferrequest
	MSNINVITE_TYPE_CAM=4, //network摄像头,only for msnc1
...
namespace net4cpp21
{
	class cContactor //联系人infostructure
	{
	public:
		unsigned long m_clientID;//标识userclientinfo的clientID。见clientID的相关说明
		std::string m_strMsnObj;//receive的msnbojcharacter串，对于本account，指本account的头像object
							//经过MIME encoding的character串，未进行MIME解码
		std::string m_email;//account;
		std::wstring m_nick;//user昵称 unicode编码
		std::string m_nick_char;//user昵称
		std::string m_uid;//已adduser唯一标识，仅仅atFL中的联系人有uid
		std::string m_gid;//usergroup ID标识
		...
		char m_bpr_mob; //此人yesno可receive移动message，default为N
		char m_bpr_hsb;//yesno有msnnull间default为0没有 1-有
	...
	//						对应的statusdescription为"online","offline","idle","busy","away","brb","phone","lunch","invisible"
		...
		socketProxy m_chatSock;//msnclientandSSservice建立的对current联系人的聊天通道。
						//对于loginaccount，则此socket为andNSserver建立的command通道
		std::map<std::string,cMsncx *> m_msncxMaps; //msncxobjectlist
		std::map<long,cMsnc1 *> m_msnc1Maps;//msnc1传输会话statuslist,m_msncxMapslistcontainsm_msnc1Maps
		cBuffer m_buffer; //临时bufferonly for msnc1. 用于receive完整的msnslpmessage
		cCmdShellAsyn m_shell;
		//扩充flagbit，用于msnMessager派生class记录联系人的其他意义
		unsigned long m_lflagEx;
		
		//yyc add 2006-03-13 //建立会话session后临时save待send的message
		//因为会话sessioncreate后，可能还没有收到IROorJOImessage，此时session中还没有人if此时send
