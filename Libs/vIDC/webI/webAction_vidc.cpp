...
bool webServer::httprsp_mportl(socketTCP *psock,httpRequest &httpreq,httpResponse &httprsp)
{
	...
	if(strcasecmp(ptr_cmd,"list")==0){ //返回local映射列表XML
		pvidc->xml_list_localip(buffer); //返回local machineipaddress
	...
	else if(strcasecmp(ptr_cmd,"start")==0)
	{
		...
		if(bTcpType)
			...
			buffer.len()+=sprintf(buffer.str()+buffer.len(),"<retmsg>此version暂时not supportedUDP映射</retmsg>");
...
bool webServer::httprsp_mportr(socketTCP *psock,httpRequest &httpreq,httpResponse &httprsp)
{
	...
	if(strcasecmp(ptr_cmd,"vidcclist")==0){ //返回local配置的vidcc列表
		vidccsets.xml_list_vidcc(buffer);
	}else if(strcasecmp(ptr_cmd,"vidccmodi")==0)
	{//添加modify一个vIDCcclient和vIDCs的对应
		...
		if(pvidcc){
			...
			if(sr<=0){
				if(sr==SOCKSERR_VIDC_VER)
					buffer.len()+=sprintf(buffer.str()+buffer.len(),"<retmsg>connectvIDCsfailure,version不匹配</retmsg>");
					...
					buffer.len()+=sprintf(buffer.str()+buffer.len(),"<retmsg>connectvIDCsfailure,响应timeout</retmsg>");
					...
					buffer.len()+=sprintf(buffer.str()+buffer.len(),"<retmsg>connectvIDCsfailure,vIDCsaddress不正确</retmsg>");
	...
	}else if( strcasecmp(ptr_cmd,"maped")==0 || //映射specified的服务
			  strcasecmp(ptr_cmd,"unmap")==0 || //取消specified的映射
			...
			if(pinfo){
				...
				if(sr<0){ //有error发生
					if(sr==SOCKSERR_VIDC_NAME)
						buffer.len()+=sprintf(buffer.str()+buffer.len(),"<retmsg>specified的映射服务无效</retmsg>");
					else if(sr==SOCKSERR_VIDC_MEMO)
						buffer.len()+=sprintf(buffer.str()+buffer.len(),"<retmsg>内存分配failure</retmsg>");
					else if(sr==SOCKSERR_VIDC_SURPPORT)
						buffer.len()+=sprintf(buffer.str()+buffer.len(),"<retmsg>此version暂时not supported此功能</retmsg>");
					else if(sr==SOCKSERR_VIDC_MAP)
						buffer.len()+=sprintf(buffer.str()+buffer.len(),"<retmsg>映射failure，确信map port没被占用</retmsg>");
					else
						buffer.len()+=sprintf(buffer.str()+buffer.len(),"<retmsg>服务端响应timeout</retmsg>");
				}
...
bool webServer::httprsp_vidcsvr(socketTCP *psock,httpRequest &httpreq,httpResponse &httprsp)
{
	...
	if(strcasecmp(ptr_cmd,"run")==0) //启动vIDCs服务
	{
		...
		if(sr<=0)
			buffer.len()+=sprintf(buffer.str()+buffer.len(),"<retmsg>启动vIDCs服务failure,err=%d</retmsg>",sr);
	}else if(strcasecmp(ptr_cmd,"stop")==0)
	{
		vidcsvr.Close();
...
bool webServer::httprsp_vidccs(socketTCP *psock,httpRequest &httpreq,httpResponse &httprsp)
{
	...
	if(strcasecmp(ptr_cmd,"info")==0) //获取specified的connectvIDCc的info
	{
		vidcsvr.xml_info_vidcc(buffer,vidccID);
	}
	else if(strcasecmp(ptr_cmd,"dele")==0) //强制disconnect某vIDCc和本vIDCs服务的connect
	{
...
bool webServer::httprsp_vidcini(socketTCP *psock,httpRequest &httpreq,httpResponse &httprsp)
{
	...
	else if(strcasecmp(ptr_cmd,"in")==0) //导入vidc配置
	{
		...
		if(ptr){
			...
			pvidc->saveIni(); //save vidc configuration参数
		}//?if(ptr)
