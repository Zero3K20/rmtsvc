...
function disp_status(xmlobj)
{
	...
	if(port>0)
	{
		...
		if(node.length>0 && (node.item(0).textContent || node.item(0).text)!="0")
			document.getElementById("lblStatus").innerHTML="<font color=green>●</font>SSL encrypted service running... &nbsp;Port:"+port;
		else	document.getElementById("lblStatus").innerHTML="<font color=green>●</font>Service running... &nbsp;Port:"+port;
	}
	else
	{
	...
	if(node.length>0) document.getElementById("lblTips").value=(node.item(0).textContent || node.item(0).text);
}
