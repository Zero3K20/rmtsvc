...
function disp_mapinfo(xmlobj)
{
	...
	if(mapped_port>0)
	{
		...
		{
			var s="&nbsp;(no client certificate required)";
			node=xmlobj.getElementsByTagName("ifsslv");
			if(node.length>0 && (node.item(0).textContent || node.item(0).text)=="1") s="&nbsp;(client certificate required)";
			document.getElementById("lblStatus").innerHTML="<font color=green>●</font>SSL encrypted service running... &nbsp;Port:"+mapped_port+s;
		}
		else	document.getElementById("lblStatus").innerHTML="<font color=green>●</font>Service running... &nbsp;Port:"+mapped_port;
		...
		document.getElementById("lblStatus").innerHTML="<font color=red>●</font>Service stopped";
...
function mapped_info()
{
	...
	if(oSelect.selectedIndex==0)
		alert("Please select a mapping service to refresh!");
	else{
		var stype="tcp";
		if(document.getElementById("tcpudp1").checked) stype="udp";
		showPopup(250, 200, 150, 20);
		// Get specified mapped port info
...
function mapped_start()
{
	var oSelect=document.getElementById("selMapped");
	if(oSelect.selectedIndex==0)
		alert("Please select a mapping service to start!");
	else{
		var stype="tcp";
		if(document.getElementById("tcpudp1").checked) stype="udp";
		showPopup(250, 200, 150, 20);
...
function mapped_stop()
{
	var oSelect=document.getElementById("selMapped");
	if(oSelect.selectedIndex==0)
		alert("Please select a mapping service to stop!");
	else{
		var stype="tcp";
		if(document.getElementById("tcpudp1").checked) stype="udp";
		showPopup(250, 200, 150, 20);
...
function mapped_dele()
{
	var oSelect=document.getElementById("selMapped");
	if(oSelect.selectedIndex==0)
		alert("Please select a mapping service to delete!");
...
function mapped_save()
{
	...
	if(idx==0)
	{
		if( !confirm("Are you sure you want to add mapping service "+mapname+"?") ) return;
	}else{
		if( !confirm("Are you sure you want to edit mapping service "+mapname+"?") ) return;
	}
	var stype="tcp";
	if(document.getElementById("tcpudp1").checked) stype="udp";
	initMapinfo();
	if(stype=="udp"){ alert("This version does not support UDP mapping"); return; }
	showPopup(250, 200, 150, 20);
