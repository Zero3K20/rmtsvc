...
public:
	...
	void close()
	{
		...
		crypto.reset();
		time_out		= 0x7fffffff;
		if(s != INVALID_SOCKET && m_owns_socket)
		{
			shutdown(s, SD_BOTH);
			closesocket(s);
		}
	...
	// Perform TLS client handshake on an already-connected socket.
	// The socket is owned externally; this object will NOT close it.
	const int connect_tls(SOCKET existing_sock, const char *host, tls_version version=tls12)
	{
		// Reset state without closing any socket
		received_close_notify = false;
		state_index = 0;
		recv_buf.clear();
		recv_channel.clear();
		...
		try
		{
			...
			// Don't close the external socket on failure
			s = INVALID_SOCKET;
			m_owns_socket = true;
			return set_err(err, -1);
		}
