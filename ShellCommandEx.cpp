...
//parse and process extended commands
BOOL doCommandEx(const char *strCmd,const char *strParam,std::string &strRet)
{
	...
	else if(strcasecmp(strCmd,"cmd")==0)//set cmd path
		bRet=SetCmdPath(strParam,strRet);	
	else if(strcasecmp(strCmd,"kill")==0) //kill specified process
		bRet=docmd_kill(strParam);
	else if(strcasecmp(strCmd,"exec")==0) //execute specified program
